...
public class UserController {
	...
	@GetMapping("/{user_id}")
	@ApiOperation(value="중복 확인",notes="입력 아이디가 이미 존재하는지 확인한다.")
	@ApiResponses({
		@ApiResponse(code = 409, message = "이미 존재하는 유저")
	})
	public ResponseEntity<? extends BaseResponseBody> checkDuplication(@PathVariable @ApiParam(value="아이디 정보", required = true) String user_id){
		...
		return ResponseEntity.status(200).body(BaseResponseBody.of(409, "이미 존재하는 유저 아이디 입니다."));
	}

	@PatchMapping("/{user_id}")
	@ApiOperation(value="유저정보 수정",notes="유저 정보를 수정한다.")
	@ApiResponses({
			@ApiResponse(code = 200, message = "성공"),
			@ApiResponse(code = 401, message = "인증 실패"),
			@ApiResponse(code = 404, message = "사용자 없음"),
	...
	@ApiOperation(value="유저정보 삭제",notes="유저 정보를 삭제한다.")
	@ApiResponses({
			@ApiResponse(code = 200, message = "성공"),
			@ApiResponse(code = 401, message = "인증 실패"),
			@ApiResponse(code = 404, message = "사용자 없음"),
